   ...
   '(progn
     ...
     (defvar *help-topics*
       (create
        ...
        (who-ps-html
         (:p "Check this to see auxiliary data near streetmap
           cursor.")
         (:p "You need to uncheck this before you can select user
           points in streetmap."))
        :aux-point-distance
        (who-ps-html
     ...
     (defun reset-layers-and-controls ()
       ...
       *images*, and put controls into pristine state."
       ...
       (reset-controls)
       (if +aux-data-p+
           (switch-phoros-controls-to "aux-data-viewer")
           (switch-phoros-controls-to "point-creator"))
       (setf *pristine-images-p* t)
       (if (and +aux-data-p+
                (checkbox-status-with-id "display-nearest-aux-data-p"))
     ...
     (defun enable-elements-of-class (class-name)
       "Activate HTML elements with class=\"class\"."
       (loop
          for element in (chain document
                                (get-elements-by-class-name class-name))
          do (setf (@ element disabled) nil)))
     ...
     (defun switch-phoros-controls-to (class-name)
       "Reveal elements of class class-name; hide anything else.
         Unless there is auxiliary data available, hide the related
         controls"
       (let ((phoros-controls-classes
              '("point-creator" "point-editor" "point-viewer"
                "multiple-points-viewer" "aux-data-viewer")))
         (dolist (c phoros-controls-classes)
           (unless (equal c class-name) (hide-elements-of-class c))))
       (reveal-elements-of-class class-name)
       (unless +aux-data-p+
         (hide-elements-of-class "aux-data-dependent")))
     ...
     (defun request-photos (&optional lonlat)
       "Set streetmap cursor to lonlat if provided.  Fetch photo
       data for a point near streetmap cursor."
       (when lonlat
     ...
     (defun request-photos-for-point ()
       "Fetch photo data near (@ *streetmap* clicked-lonlat); set or
           ...
           (when (and (@ photo-parameters usable)
              ...
              *pristine-images-p*
              (progn
                ...
                ;; TODO:
                ;; There's something in the following line that
                ;; restores layer "User Point" and removes layer
                ;; "Active Point" when coming from directly a
                ;; point-editor situation.
                (chain *streetmap* user-points-select-control (unselect-all))
                (loop
                   for i across *images* do
       ...
       (let ((selected-features-count
         ...
         (cond
           ((> selected-features-count 1)
            (switch-phoros-controls-to "multiple-points-viewer"))
           ((= selected-features-count 1)
            ...
            (setf (inner-html-with-id "aux-text-list")
                  (html-table
                   ...
                   +aux-text-labels+))
            (switch-phoros-controls-to "point-editor")
            (if (write-permission-p
                ...
                (progn
                  (disable-element-with-id "finish-point-button")
                  (disable-element-with-id "delete-point-button")
                  (switch-phoros-controls-to "point-viewer")))
     ...
     (defun flip-aux-data-inclusion ()
       "Query status of checkbox include-aux-data-p and act accordingly."
     ...
     (defun flip-nearest-aux-data-display ()
       "Query status of checkbox include-aux-data-p and act accordingly."
       (reset-layers-and-controls))
