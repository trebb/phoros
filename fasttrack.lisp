...
(defvar *rear-view-image-ping* nil)
(defvar *front-view-image-ping* nil)

(defparameter *caching-images-p* nil)
...
(defun display-date-and-image (time-widget img-widget draw-widget spinner-widget image-data)
  "Display image and its trigger time on UI."
...
(defun display-image-arrow (draw-widget image-arrow-coordinates station)
  "Display a station marker in the image on UI."
  ...
  (let (global-image-data global-image-arrow-coordinates global-image-done time-widget spinner-widget draw-widget img-widget)
    (ecase view-direction
      (:rear-view
       ...
       (setf global-image-ping '*rear-view-image-ping*)
      ...
      (:front-view
       ...
       (setf global-image-ping '*front-view-image-ping*)
                  ...
                  (block image-output
                    (if (image-data-equal current-image-data image-data)
                        (if (and (eql current-station station)
                            ...
                            (progn      ;same image; station unchanged
                              (sleep .1)
                              (bt:thread-yield)
                              (return-from image-worker))
                            (progn      ;same image; new station
  ...
  (handler-case
	...
	(pipeglade-out "zeb_network_scale" "set_font_size" 1 10)
        (dolist (image '("img_rearview" "img_frontview"))
          (pipeglade-out image "set_from_file" "public_html/phoros-logo-background.png"))
        (dolist (drawing-area '("draw_rearview" "draw_frontview"))
          (pipeglade-out drawing-area "set_source_rgba" 1 *cursor-color*)
          (pipeglade-out drawing-area "set_line_cap" 1 "round")
          (pipeglade-out drawing-area "set_line_width" 1 2)
          (pipeglade-out drawing-area "set_font_size" 1 10)
	...
	(with-open-file (in *pipeglade-in-fifo*
	  ...
	  (with-statusbar-message "starting browser"
	    (uiop:run-program (format nil "firefox '~A' &" *phoros-url*)))
          (setf *rear-view-image-ping* t)
          (setf *front-view-image-ping* t)
	  (loop
		 ...
		 ((message-name= "station_scale" message)
                  (setf *station* (parse-integer (message-data message) :junk-allowed t)) ;picked up by jump-to-station-worker
		 ...
		 ((message-name= "road_section_ok" message)
		  (digest-road-section-raw-data)
                  (prepare-chart))
		 ((message-name= "road_section_cache" message)
                  (digest-road-section-raw-data)
                  (cache-images))
                 ((message-name= "road_section_cncl" message)
		  (restore-road-section-selection)
                  (setf *caching-images-p* nil)
		 ...
		 ((message-name= "phoros" message)
		  (run-phoros-browser))
                 ((message-name= "img_rearview" message)
                  (setf *rear-view-image-ping* t))
                 ((message-name= "img_frontview" message)
                  (setf *front-view-image-ping* t))
		 (t
...
(defun digest-road-section-raw-data ()
  (when (and *postgresql-road-network-credentials* *postgresql-road-network-ok*)
    (let* ((sections (sections (make-symbol (db-credentials-table *postgresql-road-network-credentials*))))
      ...
      (save-road-section-selection)
      (unless (find sections-current *road-section-selection*)
        (set-road-section :direction :first)
        (save-road-section)
        (update-station 0))
...
(defun set-road-section (&key direction)
  (let* ((table (make-symbol (db-credentials-table *postgresql-road-network-credentials*)))
    ...
    (cond ((not *road-section-selection*)
           (invalidate-road-section))
          ((eq direction :predecessor)
...
current database."
  (if *road-section*
      ...
      (progn
        (clear-date-image-and-arrow "rear_view_time" "img_rearview" "draw_rearview")
        (clear-date-image-and-arrow "front_view_time" "img_frontview" "draw_frontview")
        (pipeglade-out "vnk" "set_text")
        (pipeglade-out "nnk" "set_text")
        (pipeglade-out "length" "set_text")
        ;; (update-station 0)
        (pipeglade-out "station" "set_text"))))
...
(defun cache-images ()
  "Download images of road-sections selected in dialog into their
canonical places."
  (unless *caching-images-p*
    (setf *caching-images-p* t)
    (bt:make-thread
     (lambda ()
       (when *postgresql-road-network-ok*
         (with-statusbar-message "caching images"
           (with-spinner "road_section_spinner"
             (handler-bind
                 ((phoros-server-error (lambda (e) (invoke-restart 'retry))))
               (with-connection *postgresql-road-network-credentials*
                 (let* ((table (make-symbol (db-credentials-table *postgresql-road-network-credentials*)))
                        (sections (sections table)))
                   (loop
                      for selected-section in *road-section-selection*
                      do
                        (cache-road-section-images (nth selected-section sections) table))))))))
       (setf *caching-images-p* nil))
     :name "cache-images")))

(let ((retry-delay 1))
  (defun cache-road-section-images (section table)
    (destructuring-bind (vnk nnk length)
        section
      (restart-case
          (progn
            (loop
               for image-data in (road-section-image-data (provenience-string *phoros-url*) table vnk nnk 10 t)
               do (if *caching-images-p*
                      (download-image image-data)
                      (loop-finish)))
            (loop
               for image-data in (road-section-image-data (provenience-string *phoros-url*) table vnk nnk 10 nil)
               do (if *caching-images-p*
                      (download-image image-data)
                      (loop-finish)))
            (setf retry-delay 1))
        (retry ()
          (with-statusbar-message (format nil "error while caching images; retry after ~A seconds" retry-delay)
            (sleep retry-delay))
          (when (< retry-delay 15)
            (incf retry-delay 1))
          (cache-road-section-images section table))))))
...
(defun launch-image (image-data image-arrow-coordinates rear-view-p)
  (if rear-view-p
      (if *show-rear-view-p*
          (progn
          ...
          (progn
            (psetf *rear-view-image-data* *empty-image-data*
                   *rear-view-image-arrow-coordinates* nil)
  ...
  (when (and vnk nnk station)
    (with-connection *postgresql-road-network-credentials*
      ...
      (let* ((table (make-symbol (db-credentials-table *postgresql-road-network-credentials*)))
        ...
        (launch-image image-data image-arrow-coordinates rear-view-p)))))
